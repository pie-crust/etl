...
if __name__=="__main__":
	...
	if 1:
		...
		tconn = pyodbc.connect("DSN=SFDDATAMART;Database=ACCOUNTINGBI;WSID=ld-dbn-bofin006;APP=AnishTest;authenticator=https://home.okta.com;uid=s_dev_racct@homegroup.com;pwd=home312!;autocommit=False;ROLE=s_dev_racct_ROLE;")
		#tconn.setdecoding(pyodbc.SQL_CHAR, encoding='utf-8')
		tcur = tconn.cursor()
		tcur.execute("USE WAREHOUSE LOAD_WH")
		tcur.execute("USE DATABASE ACCOUNTINGBI")
	...
	sconn = pyodbc.connect(u'DSN=MDIR01;Database=DataStaging;Trusted_Connection=yes;POOL=0;App=FiccApi')	
	scur = sconn.cursor()
