...
params[ 'DY_FinancingPosition' ] = [ '{CLIENT} {YEAR}/{MONTH}/{DAY} {YEAR}/{MONTH}/{DAY} EOD EOD "*" "*" "*" NONE "*" NOW 1' ]
params[ 'ME_FinancingPosition' ] = [ '{CLIENT} {YEAR}/{MONTH}/{EOM} {YEAR}/{MONTH}/{EOM} EOD EOD "*" "*" "*" NONE "*" NOW 1' ]
...
def get_cli(a):
	...
	return 'time %s cli.py -nopp {PARAM_CNT} {DUMP} -dcf %s -pcf %s/{TABLE}.json {LAME_DUCK} -rte {RUNTIME} --proc_params' % (PY, cfg, cloc)
@click.command()
...
def main(**kwargs):
	...
	cgroup_fp_dy = [ 'DY_FinancingPosition']
	cgroup_fp_me = [  'ME_FinancingPosition']
	if a.table in cgroup_dy + cgroup_me + cgroup_fp_dy+ cgroup_fp_me:
		if a.table not in cgroup_fp_dy+ cgroup_fp_me:
...
def load_day(a, mon, day=None):
	...
	pycli  = cli.format(**dict( TABLE = a.table, PARAM_CNT = len([p for p in pars.strip().split(' ') if p]), DUMP = '--dump' if a.dump else '--no-dump', LAME_DUCK='-ld %d ' % a.lame_duck, RUNTIME=a.runtime_env))
	cmd= '%s %s' % (pycli, pars)
