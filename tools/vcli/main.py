		...
		def one_iteration(text=None):
			...
			if text is None:
				...
				try:
					...
					logger.error("1 traceback: %r", traceback.format_exc())
					self.echo(str(e), err=True, fg="red")
					return
			...
			try:
				...
				if 1:
					...
					#logger.error("sql: %r, error: %r", text, e)
					logger.error("2 traceback: %r", traceback.format_exc())
					self.echo(str(e), err=True, fg="red")
			except Exception as e:
				#logger.error("sql: %r, error: %r", text, e)
				logger.error("3 traceback: %r", traceback.format_exc())
