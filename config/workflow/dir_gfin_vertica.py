				...
				for _trg_class, val in cli.cfg['target'][_source].items():
					...
					for okfn in ok_files.file_names:
						...
						if 1:
							...
							toDB.load_gfin_file ( trans	= to_conn, file_obj = outFile, schema = schema, table_name=masterTbl,  qname = 'insertStmt', fmt_cols=fmt_cols, cfg = (dir_scfg, tcfg), skip=skip, apx=apx, stats=stats)
						...
						for out_fn in [x for x in out_files.file_names if not os.path.basename(x).split('.')[1] in [masterTabTag]]:
							...
							if tbl not in [masterTabTag] +do_not_load: 
								...
								if 1:
									...
									outFile.set_alt_cols()
									toDB.load_gfin_file ( trans	= to_conn, file_obj = outFile, schema = schema, table_name=tbl,  qname = 'insertStmt', fmt_cols = fmt_cols,  cfg = (dir_scfg, tcfg), skip=skip, apx=apx, stats=stats)
									loaded[out_fn] = tbl
							else:
