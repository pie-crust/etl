...
def daemon(data):
	print('Starting %s' % len(data.data))
	time.sleep(5)
	print('Exiting  %s' % len(data.data))
...
def run():	
	...
	tid=0
	while iq_data.data:
		threads[tid]=threading.Thread(target=daemon, kwargs={'data':iq_data})
		threads[tid].start()
		IQ.fetch_next(out=iq_data)
		
		if 0:
			threads[tid]=threading.Thread(target=S3StreamLoader.upload_data, kwargs={'data':iq_data, 'target':cli.s3cfg, 'out':s3_file_names})
			threads[tid].start()
			IQ.fetch_next(out=iq_data)
			tid +=1
