"""
time python cli.py -nopp 2 --no-dump -no-mf test/mock/DY_FiccDistribution/15_h.csv -dcf config/db_config.json -pcf config/proc/sql_snow/DY_FiccDistribution.json --proc_params \
'2018-12-31 00:00:00' "$(date +"%Y-%m-%d %H:%M:%S")"
...
#Conventional insert
 time python cli.py -nopp 2 --no-dump -no-mf test/mock/DY_FiccDistribution/15_h.csv -dcf config/db_config.json \
 -pcf config/proc/sql_snow_insert/DY_FiccDistribution.json \
...
class sql_fetchmany_s3_snow(Cli):
	...
	def proc_config(self):
		...
		log.info("Table: %s '%s'" % (self.stab, "','".join(self.pa)))
