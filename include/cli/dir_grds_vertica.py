...
class dir_grds_vertica(Cli):
	...
	def get_dest_folder (self, okdir):
		...
		return dest_folder
	@ctimeit
	def createPrimaryKeyFile(self, ffObj, pkfn):
		cnt=0
		with open(pkfn, 'wb') as fh: 
			for line in ffObj.get_columns( cols = [0], skip = 0):
				#print line
	...
	def proc_config(self):
		...
		if isinstance(self.pa, list):
			...
			assert self.pa
			assert os.path.isdir(self.pa), 'Snap dir [%s] does not exists.' % self.pa
		self.pa=[self.pa]
