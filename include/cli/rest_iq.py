...
from include.fmt import pfmt, ppe
...
class rest_iq(Cli):
	...
	def	get_default_acct_date(self, DB, tcfg):
		me_stmt=cli.get_parsed(ckey='monthEndStmt', cfg=tcfg)
		cur = DB.exec_query(me_stmt)
		data = cur.fetchall()
		
		assert len(data)==1
		assert len(data[0])==1
		fmt = cli.get_parsed(ckey='accountingDateFmt', cfg=tcfg) 
		out= data[0][0].strftime(fmt)

		return out
	def	set_default_acct_date(self, DB, tcfg):
		
		acct_date=cli.get_parsed(ckey='accountingDate', cfg=tcfg)
		if str(acct_date).lower() in ['auto']:
			ddate = self.get_default_acct_date(DB, tcfg)
			self.pa[0]=ddate
			acct_date=cli.get_parsed(ckey='accountingDate', cfg=tcfg)
		assert str(acct_date).lower() not in ['auto']
	...
	def usage(self):
		if 1:
			...
			print ('#'*80)
			print ("""Usage:
  time python cli.py -nopp 2  -dcf config/db_config.json -pcf config/proc/rest_iq/DY_FinancingPosition.json \
 --proc_params  Repo '2018-12-31 00:00:00' $GA_TOCKEN
#
# pa[0] = AccountingDate
# pa[1] = gatoken
#
