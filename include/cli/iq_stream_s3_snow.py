...
class iq_stream_s3_snow(Cli):
	...
	def get_s3cfg(self, cname=None):
		if not cname:
			if self._source:
				return self.cfg['s3'][self._source]
			else:
				return self.cfg['s3']
		else:
			assert cname
			assert  self._source
			self._s3_class = cname
			return self.cfg['s3'][self._source][self._s3_class]
	@ctimeit
	def proc_config(self):
		...
		if self.s3cfg:
			...
			self.to_dir=self.s3cfg['targetDir']
			...
			if not int(self.s3cfg["maxRecordsInFile"])>= int(self.s3cfg["writeBufferSize"]):
				self.s3cfg["writeBufferSize"]=self.s3cfg["maxRecordsInFile"]
			if 1:
				###############
				self.max_rows_to_read=int(self.s3cfg["maxRecordsInFile"])
