...
"""
	...
	def proc_config(self):
		...
		if 0:
			...
			for src in tear['source']:
				...
				self._source=src
				self.tdir_path[src] = self.get_parsed(ckey='targetDir', cfg=dcfg[src])
	...
	def get_log_stats(self, ll, cli_stats):
		...
		('total_saved', 'Total records saved: (?P<total_saved>[0-9,.]+)'),
		]	
